...
export function Fixtures() {
  ...
  useEffect(() => {
    if (gameweeks && gameweeks.length > 0 && !selectedGwId) {
      const activeGw =
        gameweeks.find((gw) => gw.status === "active") ||
  ...
  const { data: fixtures, isLoading: isLoadingFixtures } =
    useGetGameweekFixtures(selectedGwId!, {
      query: { enabled: !!selectedGwId },
    });

  // Load API-Sports widget script once
  useEffect(() => {
    const existing = document.querySelector('script[src*="api-sports"]');
    if (!existing) {
          ...
          {isLoadingFixtures ? (
            ...
            <div className="divide-y divide-border">
              {fixtures.map((fixture) => (
                <div
                  key={fixture.id}
                  className="p-4 sm:p-6 flex flex-col sm:flex-row items-center justify-between hover:bg-secondary/10 transition-colors gap-4"
                >
                  ...
                  <div className="flex flex-col items-center justify-center min-w-[120px] shrink-0">
                    {fixture.status === "scheduled" ? (
                      ...
                      <div
                        className={`px-4 py-2 rounded-md font-mono text-xl font-bold flex gap-3 border ${fixture.status === "live" ? "bg-primary/10 border-primary text-primary shadow-[0_0_10px_rgba(var(--primary),0.2)]" : "bg-secondary border-border"}`}
                      >
                    ...
                    {fixture.status === "live" && (
                      <Badge
                        variant="destructive"
                        className="mt-2 text-[10px] uppercase font-bold animate-pulse"
                      >
                        Live
                      </Badge>
                    )}
                    ...
                    {fixture.status === "scheduled" && (
                      <span className="text-xs text-muted-foreground mt-2">
                        {format(new Date(fixture.kickoff), "EEE, d MMM")}
                      </span>
                    )}
                  </div>
